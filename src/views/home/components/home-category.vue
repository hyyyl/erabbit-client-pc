<template>
  <div class='home-category' @mouseleave="categoryId = ''">
    ...
    <!-- 弹层 -->
    <div class="layer">
      <h4>{{currentCategory && currentCategory.id === 'brand' ? '品牌推荐' : '商品推荐'}} <small>根据您的购买或浏览记录推荐</small></h4>
      <!-- 商品 -->
...
export default {
  ...
  setup () {
    ...
    // 得到弹出层的推荐商品数据(ref响应式数据，所以需要.value)
    const categoryId = ref(null)
    const currentCategory = computed(() => menuList.value.find(item => item.id === categoryId.value))
